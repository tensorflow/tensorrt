...
if __name__ == "__main__":
    ...
    infer_fn_concrete = tf.function(model.__call__).get_concrete_function({
        "input_ids": tf.TensorSpec([None, model.config.n_positions], dtype=tf.int32, name="input_ids"),
        "attention_mask": tf.TensorSpec([None, model.config.n_positions], dtype=tf.int32, name="attention_mask"),
        "token_type_ids": tf.TensorSpec([None, model.config.n_positions], dtype=tf.int32, name="token_type_ids"),
    })
    ...
    print(f"Saving `{args.model_name} tokenizer` in directory: `{tokenizer_dir}`.")
